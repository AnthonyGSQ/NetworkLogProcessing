...
TEST(ApplicationTest, GoodConstructor) {
    ...
    std::thread server_thread([&app, &sync_point]() {
        ...
        try {
            ...
            std::cerr
                << "[Test] Expected behavior - server initiation handled: "
                << e.what() << "\n";
        }
    });
...
TEST(ApplicationTest, BadConstructor) {
    ...
    // Invalid port in constructor - HttpServer constructor will validate and
    // throw
    EXPECT_THROW(Application app2(".env", -100), std::runtime_error);
}
...
TEST(ApplicationTest, RunFailsPortInUse) {
    ...
    // Try to create and run second application on same port - should fail in
    // run()
...
TEST(ApplicationTest, InvalidConfigThrows) {
    EXPECT_THROW(Application app("nonexistent_file.env", 9094),
                 std::runtime_error);
}

// Test that zero/negative port throws in constructor
TEST(ApplicationTest, InvalidPortThrows) {
    EXPECT_THROW(Application app(".env", 0), std::runtime_error);

    EXPECT_THROW(Application app2(".env", -1), std::runtime_error);

    EXPECT_THROW(Application app3(".env", -9999), std::runtime_error);
}

...
TEST(PostgresDB, OverlappingReservationsRejected) {
    ...
    EXPECT_FALSE(db.insertReservation(res2)) << "Overlapping reservations should be rejected";
...
TEST(PostgresDB, BoundaryOverlapRejected) {
    ...
    // Try Room 107, Feb 18-20 (checkout=checkin boundary, overlap) - SHOULD FAIL
    Reservation res2 = createBaseReservation();
    res2.room_number = 107;
    res2.check_in_date = "2026-02-18";
    res2.check_out_date = "2026-02-20";
    EXPECT_FALSE(db.insertReservation(res2)) << "Boundary overlap should be rejected";

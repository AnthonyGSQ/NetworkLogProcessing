...
Reservation Logger::parseJson(const std::string& jsonFile) {
    ...
    try {
        ...
        // Guest data
        currentReservation.guest_name = currentJson.at("guest_name").as_string();
        currentReservation.guest_email = currentJson.at("guest_email").as_string();
        currentReservation.guest_phone = currentJson.at("guest_phone").as_string();
        
        // Reservation info
        currentReservation.room_number = currentJson.at("room_number").as_int64();
        currentReservation.room_type = currentJson.at("room_type").as_string();
        currentReservation.number_of_guests = currentJson.at("number_of_guests").as_int64();
        
        // Dates
        currentReservation.check_in_date = currentJson.at("check_in_date").as_string();
        currentReservation.check_out_date = currentJson.at("check_out_date").as_string();
        currentReservation.number_of_nights = currentJson.at("number_of_nights").as_int64();
        
        // Cost
        currentReservation.price_per_night = currentJson.at("price_per_night").as_double();
        currentReservation.total_price = currentJson.at("total_price").as_double();
        currentReservation.payment_method = currentJson.at("payment_method").as_string();
        ...
        if (currentJson.contains("reservation_status")) {
            currentReservation.reservation_status = currentJson.at("reservation_status").as_string();
        }
        
        // Special requests (opcional)
        if (currentJson.contains("special_requests")) {
            currentReservation.special_requests = currentJson.at("special_requests").as_string();
        }
        ...
        // if any data of the reservation is invalid/void, throws a invalid argument err
...
bool Logger::validateJsonFormat(const Reservation& reservation) {
    ...
    if (reservation.guest_email.empty() || reservation.guest_email.find('@') == std::string::npos) {
    ...
    // Validar fechas
    if (reservation.check_in_date.empty() || reservation.check_out_date.empty()) {
